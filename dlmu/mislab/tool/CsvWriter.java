...
//import dlmu.mislab.common.KeyObjectPair;

/**
 * A stream based writer for writing delimited text data to a file or a stream.
 */
public class CsvWriter {
	private SimpleEntry<String,String>[] headerAlias;
	...
	public CsvWriter(String fileName) {
		...
		this(fileName, Letters.COMMA, Charset.forName("GBK"));
	}
	...
	/***
	 * 
	 * By GuRui on 2016-6-16 下午3:28:00
	 * @return
	 */
	public SimpleEntry<String,String>[] getHeaderAlias() {
		return headerAlias;
	}

	/***
	 * A list contains the alias of headers
	 * By GuRui on 2016-6-16 下午3:28:05
	 * @param headerAlias
	 */
	public void setHeaderAlias(SimpleEntry<String,String>[] headerAlias) {
	...
	/***
	 * Save a list of key-object-pair list into a csv file (Open for excel) 
	 * By GuRui on 2016-6-16 下午1:58:28
	 * @param filename : the csv filename to be created
	 * @param list : a list of key-object-pair list. The header row will be auto-generated
	 * @param headerAlias : if set, then use these alias in header; otherwise, key name is used (default)
	 * @throws IOException
	 */
	public void saveCsv(String filename, List<List<SimpleEntry<String, Object>>> list) throws IOException{
		this.saveCsv(filename,list,true);
	}
	
	/***
	 * Save a list of key-object-pair list into a csv file 
	 * By GuRui on 2016-6-16 下午1:58:28
	 * @param filename : the csv filename to be created
	 * @param list : a list of key-object-pair list. The header row will be auto-generated
	 * @param forExcel : is value store in an excel formula format which were to be correctly displayed by excel. If the csv file to be opened by excel, set this flag to true. Otherwise, false;
	 * @param headerAlias : if set, then use these alias in header; otherwise, key name is used (default)
	 * @throws IOException
	 */
	public void saveCsv(String filename, List<List<SimpleEntry<String, Object>>> list, boolean forExcel) throws IOException{
		if(list==null || list.size()==0){
			this.writeComment("Empty");
			this.close();
			return;
		}
		...
		for(int i=0; i<list.get(0).size();i++){
			header[i]=list.get(0).get(i).getKey();
		}
		
		if(this.headerAlias!=null && this.headerAlias.length>0){
			for(SimpleEntry<String,String> s : this.headerAlias){
				for(int j=0; j<header.length; j++){
					if(s.getKey().equalsIgnoreCase(header[j])){
						header[j]=s.getValue();
						break;
					}
				}
		...
		this.writeRecord(header,false);
		for(List<SimpleEntry<String, Object>> row : list){
			String[] contents=new String[size];
			for(int i=0; i<size;i++){
				Object val=row.get(i).getValue();
				if(val!=null){
					if(forExcel){
						contents[i]= "=\"" + val.toString() + "\"";
					}else{
						contents[i]=val.toString();
					}
				}else{
